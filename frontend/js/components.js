const ui = {
    ...
    renderNavbar(activePage = 'home') {
        ...
        let adminTag = user && user.role === 'admin' 
            ? '<span class="text-[10px] bg-slate-800 text-white px-2 py-0.5 rounded ml-2 align-middle">ADMIN</span>' 
            : '';
        ...
        const navHtml = `
            <nav class="navbar-glass sticky top-0 z-40 p-4">
                <div class="max-w-7xl mx-auto flex justify-between items-center relative">
                    ...
                    <!-- ✅ Safe mobile toggle -->
                    <button onclick="(function(){ const m=document.getElementById('mobile-menu'); if(m) m.classList.toggle('hidden'); })()" class="md:hidden text-2xl px-2">
                        ☰
                    </button>
                </div>
